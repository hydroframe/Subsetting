...
class RegressionClipTests(unittest.TestCase):
    ...
    def test_subset_dem_to_tif_conus1(self):
        ...
        clipper = MaskClipper(subset_mask=mask_file, no_data_threshold=-1)
    ...
    def test_box_clip(self):
        ...
        subset, _, _, _ = box_clipper.subset(data_array)
        ...
        subset2, _, _, _ = box_clipper.subset(data_array)
        ...
        subset3, _, _, _ = box_clipper.subset(data_array)
        ...
        subset4, _, _, _ = box_clipper.subset(data_array)
        ...
        box_clipper2 = BoxClipper()
        subset5, _, _, _ = box_clipper2.subset(data_array2)
        ...
        subset6, _, _, _ = box_clipper2.subset(data_array2)
    ...
    def test_box_clip_invalid_nx_dim(self):
        with self.assertRaises(Exception):
            ...
            #data_array = file_io_tools.read_file(test_files.conus1_dem.as_posix())
            BoxClipper(nx=0, ny=500)

    def test_box_clip_invalid_x_dim(self):
        with self.assertRaises(Exception):
            #data_array = file_io_tools.read_file(test_files.conus1_dem.as_posix())
            BoxClipper(nx=15, x=0)
